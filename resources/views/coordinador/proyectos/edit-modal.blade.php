    ...
    <div class="flex min-h-screen items-center justify-center p-4 sm:p-6 lg:p-8">
        <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-3xl mx-auto overflow-hidden transform transition-all duration-300 scale-100 max-h-[90vh] flex flex-col">
            ...
            <form id="form-editar-proyecto" action="{{ (old('_method') === 'PUT' && session('edit_proyecto_id')) ? '/coordinador/proyectos/' . session('edit_proyecto_id') : '' }}" method="POST" class="px-8 py-6 space-y-5 overflow-y-auto scrollbar-thin flex-1">
                ...
                <div>
                    <label for="edit-estudiante" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Alumno Asignado al Proyecto <span class="text-gray-400 font-normal lowercase">(opcional)</span></label>
                    <div class="relative">
                        <select id="edit-estudiante" name="estudiante_id" class="block w-full px-4 py-3 bg-gray-50/50 border border-gray-200 focus:border-[#6BA53A] focus:ring-1 focus:ring-[#6BA53A] rounded-xl text-sm font-medium text-gray-800 shadow-sm transition-all appearance-none cursor-pointer @error('estudiante_id') border-red-400 bg-red-50 @enderror">
                            <option value="">Sin Asignar / Selecciona un alumno...</option>
                            @foreach($alumnos as $al)
                                <option value="{{ $al->id }}" {{ (old('_method') === 'PUT' ? old('estudiante_id') : '') == $al->id ? 'selected' : '' }}>
