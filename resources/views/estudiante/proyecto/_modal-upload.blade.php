...
<div id="uploadModal" class="hidden fixed inset-0 z-[99] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl shadow-2xl border border-gray-200 max-w-md w-full overflow-hidden fade-in-up">
        ...
        <form action="{{ route('estudiante.subirDocumento') }}" method="POST" enctype="multipart/form-data" id="uploadDocumentForm">
            @csrf
            @if(isset($solicitud))
                <input type="hidden" name="solicitud_id" value="{{ $solicitud->id }}">
            @endif
            <input type="hidden" name="nombre_doc" id="uploadDocNameInput" value="">

            {{-- Modal header --}}
            <div class="bg-gradient-to-r from-[#4E7D24] to-[#6BA53A] p-5 text-white flex justify-between items-center">
                <div>
                    <h3 class="text-lg font-bold">Subir Documento</h3>
                    <p class="text-xs text-white/80 mt-0.5" id="uploadModalDocName">Cargando...</p>
                </div>
            ...
            <div class="p-6 space-y-4">
                <div class="border-2 border-dashed border-gray-300 rounded-2xl p-8 flex flex-col items-center justify-center text-center hover:border-[#6BA53A] transition-colors cursor-pointer"
                    ...
                    <input type="file" id="simPdfInput" name="archivo" accept=".pdf" class="hidden" onchange="fileSelected(this)">
